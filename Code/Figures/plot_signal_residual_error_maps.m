...
switch multisample
    ...
    case false
        signals = load(fullfile(projectfolder, 'Outputs', 'ESL Signal Estimation', SampleName, 'signals.mat')).signals;
        signals = squeeze(signals(:,seriesindx,1));
end
...
error = (ImageArray-pred).*double(pred>0);
szmap = size(pred);
...
disp(max(abs(squeeze(error_large(sl,xs,ys))), [], 'all'))
ax2 = axes;
...
saveas(f1, fullfile(projectfolder, 'Figures', ['Signal Residual Error b' num2str(bval) ' Delta' num2str(DELTA) '_' snum '.png']))
