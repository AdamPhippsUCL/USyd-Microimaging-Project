% Script to estimate ESL signals
...
for sindx = 1:length(SampleNames)
    ...
    % NEW: Select voxels IN BENIGN SAMPLES AND WITH NON-ZERO composition
...
for imgindx = 1:Nimg
    ...
    signals(:,imgindx,4) = prctile(BootFits,99.5); % 97.5th percentile

    % RESULTS
    RESULTS(imgindx).E = [signals(1, imgindx, 1), signals(1, imgindx, 3), signals(1, imgindx, 4)];% [num2str(signals(2, imgindx, 1)) ' (' num2str(signals(2, imgindx, 2)) ')'];
    RESULTS(imgindx).S = [signals(2, imgindx, 1), signals(2, imgindx, 3), signals(2, imgindx, 4)];% [num2str(signals(1, imgindx, 1)) ' (' num2str(signals(1, imgindx, 2)) ')'];
